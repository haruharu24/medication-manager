...
const ScoreChart: React.FC<{ data: { date: string, score: number }[] }> = ({ data }) => {
  ...
  if (data.length < 2) return (
    <div className="h-[150px] flex items-center justify-center bg-slate-50 rounded-[32px] border border-dashed border-slate-200">
      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">推移を表示するには2日以上の記録が必要です</p>
    </div>
  );
  ...
  return (
    <div className="bg-slate-50 p-6 rounded-[32px] border border-slate-100 overflow-x-auto no-scrollbar">
...
export const ReportPreviewView: React.FC<ReportPreviewViewProps> = ({ config, medications, logs, conditions, globalLogs, onBack }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-white overflow-y-auto pb-32 print:p-0">
      <div className="p-4 safe-top flex items-center justify-between border-b print:hidden sticky top-0 bg-white/90 backdrop-blur-xl z-50">
        <button onClick={onBack} className="p-2 text-slate-600"><ChevronLeft size={24}/></button>
        <div className="flex gap-2">
          <button onClick={handleShare} className="p-3 bg-slate-100 text-slate-600 rounded-full active:scale-90 transition-transform"><Share size={20}/></button>
          <button onClick={() => window.print()} className="bg-slate-900 text-white px-6 py-2 rounded-full font-black text-sm flex items-center gap-2 shadow-lg"><Printer size={16}/> PDF出力 / 印刷</button>
        </div>
      </div>

      <div className="p-10 space-y-12 max-w-2xl mx-auto w-full print:p-0 print:max-w-none">
        <div className="text-center space-y-2 border-b-8 border-slate-900 pb-10">
          <h2 className="text-4xl font-black text-slate-900 tracking-tighter italic uppercase">Report</h2>
          <p className="text-sm font-black text-slate-400 uppercase tracking-[0.2em]">{config.start.replace(/-/g, '/')} — {config.end.replace(/-/g, '/')}</p>
        </div>
        ...
        {config.includeCondition && (
          <section className="space-y-8">
            ...
            <h3 className="text-2xl font-black text-slate-900 flex items-center gap-3 border-b-2 border-slate-100 pb-4 mt-12">
            ...
            <div className="space-y-4">
              {dateRange.slice().reverse().map(date => {
                ...
                return (
                  <div key={dateStr} className="flex gap-6 p-6 bg-slate-50 rounded-[32px] border border-slate-200">
                    <div className="w-16 shrink-0 text-center">
                      <p className="text-sm font-black text-slate-900">{format(date, 'MM/dd')}</p>
                      <p className="text-[10px] font-bold text-slate-400">{format(date, '(EE)', { locale: ja })}</p>
                    </div>
                    <div className="flex-1 space-y-2">
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] font-black text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full uppercase tracking-widest">SCORE: {cond.score}</span>
                      </div>
                      <p className="text-sm text-slate-600 leading-relaxed font-medium">「{cond.memo || '特記事項なし'}」</p>
                    </div>
                  </div>
                );
              })}
        ...
        <div className="text-[10px] text-slate-300 text-center font-bold uppercase tracking-widest pt-20">
...
const StatBox = ({ label, value, color }: any) => (
  <div className={`bg-slate-50 p-8 rounded-[40px] text-center border-2 border-slate-100`}>
