...
export const MedicationListView: React.FC<MedicationListViewProps> = ({ 
  ...
  const renderMedItem = (med: Medication) => (
    <div 
      ...
      className="bg-slate-50/50 p-4 rounded-2xl border border-slate-100 flex justify-between items-center mb-2 last:mb-0 active:bg-slate-100 transition-colors"
    >
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50">
      <div className="bg-slate-50 py-3 safe-top border-b border-slate-200 relative">
        <h1 className="text-center font-bold text-slate-800">お薬ボックス</h1>
      ...
      <div className="px-5 py-3 bg-white border-b border-slate-100 flex justify-center">
        <div className="flex bg-slate-100 rounded-xl p-1 w-full max-w-xs">
          <button 
            onClick={() => setTab('list')} 
            className={`flex-1 py-2 text-sm font-bold rounded-lg flex items-center justify-center gap-2 transition-all ${tab === 'list' ? 'bg-white shadow-sm text-emerald-700' : 'text-slate-400'}`}
          >
            <Pill size={16}/> リスト
          </button>
          <button 
            onClick={() => setTab('history')} 
            className={`flex-1 py-2 text-sm font-bold rounded-lg flex items-center justify-center gap-2 transition-all ${tab === 'history' ? 'bg-white shadow-sm text-emerald-700' : 'text-slate-400'}`}
          >
            <History size={16}/> 履歴
          </button>
        </div>
          ...
          <div className="p-4 space-y-6">
                ...
                <div key={folder.id} className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h2 className="font-black text-slate-800 tracking-tight">{folder.title}</h2>
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{children.length} 項目</p>
                      </div>
                    </div>
                    <ChevronDown size={18} className={`text-slate-300 transition-transform duration-300 ${isExpanded ? 'rotate-180' : ''}`} />
                  </div>
                  {isExpanded && (
                    <div className="px-4 pb-4 space-y-2 border-t border-slate-50 pt-2 bg-slate-50/30">
            ...
            {orphanMeds.length > 0 && (
              <div className="space-y-3">
                <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-2">個別のお薬</h3>
                {orphanMeds.map(med => (
                  <div 
                    key={med.id} 
                    onClick={() => onEditMed(med)} 
                    className="bg-white p-5 rounded-[24px] border border-slate-100 flex justify-between items-center shadow-sm active:scale-98 transition-transform"
                  >
                    <div className="flex items-center gap-4">
          ...
          <div className="p-4 space-y-4">
            {globalLogs.length === 0 ? (
              <div className="py-20 text-center space-y-2">
                ...
                <p className="text-slate-400 font-bold text-sm">履歴はありません</p>
              </div>
            ) : (
              globalLogs.map(log => (
                <div key={log.id} className="bg-white p-4 rounded-[24px] border border-slate-100 shadow-sm flex gap-4 items-start">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${
                    log.type === 'add' ? 'bg-emerald-50 text-emerald-500' : 
                    log.type === 'update' ? 'bg-blue-50 text-blue-500' : 
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between items-start mb-1">
                      ...
                      <span className="text-[9px] font-bold text-slate-400 bg-slate-50 px-1.5 py-0.5 rounded uppercase tracking-tighter">
