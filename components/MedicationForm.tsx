  ...
  return (
    <div className="relative h-[160px] w-20 overflow-hidden select-none">
      <div ref={containerRef} onScroll={handleScroll} className="h-full overflow-y-auto no-scrollbar snap-y snap-mandatory" style={{ paddingTop: PADDING, paddingBottom: PADDING }}>
        {items.map((item) => <div key={item} className="h-[40px] flex items-center justify-center snap-center text-xl font-medium text-slate-300 dark:text-slate-600">{format ? format(item) : item}</div>)}
      </div>
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-50 dark:bg-slate-900 flex flex-col animate-in slide-in-from-bottom duration-300">
      <div className="bg-white dark:bg-slate-800 px-4 py-3 flex items-center justify-between shadow-sm border-b border-slate-200 dark:border-slate-700 safe-top">
        <button onClick={onCancel} className="p-2 -ml-2 text-slate-900 dark:text-white hover:bg-slate-100 dark:hover:bg-slate-700 rounded-full transition-colors"><ChevronLeft size={24} /></button>
        <div className="flex flex-col items-center">
          <h2 className="text-sm font-black text-slate-800 dark:text-slate-100 tracking-tight">{isFolder ? 'グループの編集' : 'お薬の編集'}</h2>
          {isFolder && <span className="text-[10px] text-slate-400 dark:text-slate-500 font-bold uppercase">{initialData?.folderType === 'one-pack' ? '一包化' : '服用グループ'}</span>}
        </div>
        <div className="w-10"></div> 
      </div>

      <div className="flex-1 overflow-y-auto p-4 pb-48 safe-bottom">
        <div className="max-w-md mx-auto space-y-4">
          
          <div className="bg-white dark:bg-slate-800 p-5 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-700">
            <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2">名前</label>
            <div className="flex items-center gap-3">
              ...
              <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="名称を入力..." className="flex-1 bg-transparent text-xl font-black text-slate-800 dark:text-slate-100 outline-none" />
            </div>
          </div>
          ...
          {!isFolder && (
            <>
              <div className="bg-white dark:bg-slate-800 p-5 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-700 relative">
                <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2">開始日（処方日）</label>
                <div className="flex items-center gap-3">
                  <CalendarIcon size={18} className="text-slate-400 dark:text-slate-500" />
                  <input type="date" value={startDateStr} onChange={(e) => setStartDateStr(e.target.value)} className="bg-transparent text-lg font-bold text-slate-800 dark:text-slate-100 outline-none flex-1" />
                </div>
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div className="bg-white dark:bg-slate-800 p-5 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-700 relative">
                  <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2">1回の量</label>
                  <select value={dosage} onChange={(e) => setDosage(e.target.value)} className="w-full bg-transparent text-lg font-bold text-slate-800 dark:text-slate-100 outline-none appearance-none">
                    {DOSAGE_OPTIONS.map(d => <option key={d} value={d}>{d}</option>)}
                  </select>
                  <ChevronDown size={14} className="absolute right-4 bottom-6 text-slate-400 dark:text-slate-500" />
                </div>
                <div className="bg-white dark:bg-slate-800 p-5 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-700 relative">
                  <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2">単位</label>
                  <select value={unit} onChange={(e) => setUnit(e.target.value as any)} className="w-full bg-transparent text-lg font-bold text-slate-800 dark:text-slate-100 outline-none appearance-none">
              ...
              <div className="bg-white dark:bg-slate-800 p-5 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-700 relative">
                <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2">タイミング</label>
                <select value={label} onChange={(e) => setLabel(e.target.value)} className="w-full bg-transparent text-lg font-bold text-slate-800 dark:text-slate-100 outline-none appearance-none">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-white dark:bg-slate-800 p-5 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-700 relative">
                  <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2">在庫</label>
                  <select value={stock} onChange={(e) => setStock(e.target.value)} className="w-full bg-transparent text-lg font-bold text-slate-800 dark:text-slate-100 outline-none appearance-none">
                    {STOCK_OPTIONS.map(s => <option key={s} value={s}>{s}</option>)}
                  </select>
                  <ChevronDown size={14} className="absolute right-4 bottom-6 text-slate-400 dark:text-slate-500" />
                </div>
                <div className="bg-white dark:bg-slate-800 p-5 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-700 relative">
                  <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2">通知</label>
                  <button onClick={() => setIsTimePickerOpen(true)} className="w-full text-left text-lg font-bold text-slate-800 dark:text-slate-100">{notificationTime || '--:--'}</button>
                  <Clock size={14} className="absolute right-4 bottom-6 text-slate-400 dark:text-slate-500" />
                </div>
              </div>

              <div className="bg-white dark:bg-slate-800 p-5 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-700 relative">
                <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2">在庫アラート(残り何回で警告)</label>
                <select value={lowStockThreshold} onChange={(e) => setLowStockThreshold(e.target.value)} className="w-full bg-transparent text-lg font-bold text-slate-800 dark:text-slate-100 outline-none appearance-none">
          ...
          <div className="bg-white dark:bg-slate-800 p-5 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-700">
            <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2">メモ</label>
            <textarea rows={3} value={memo} onChange={(e) => setMemo(e.target.value)} placeholder="特記事項があれば..." className="w-full bg-transparent text-sm font-medium text-slate-600 dark:text-slate-300 outline-none resize-none" />
          </div>

          {onDelete && (
            <button type="button" onClick={() => setShowDeleteModal(true)} className="w-full py-5 bg-red-50 dark:bg-red-500/10 text-red-500 rounded-3xl font-black text-sm uppercase tracking-widest flex justify-center items-center gap-2">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-[40px] shadow-2xl w-full max-w-[320px] relative z-10 animate-in zoom-in-95 overflow-hidden">
            <div className="p-8 flex justify-center items-center gap-4 relative">
              ...
              <span className="text-2xl font-black text-slate-300 dark:text-slate-600 pb-1">:</span>
            ...
            <div className="p-4 bg-slate-50 dark:bg-slate-900 border-t border-slate-100 dark:border-slate-700 flex gap-3">
              <button onClick={() => { setNotificationTime(''); setIsTimePickerOpen(false); }} className="flex-1 py-4 text-red-500 font-bold text-sm">解除</button>
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-[40px] p-8 w-full max-w-sm relative z-10 text-center">
            <div className="w-20 h-20 bg-red-100 dark:bg-red-500/15 text-red-500 rounded-3xl flex items-center justify-center mx-auto mb-6"><Trash2 size={40}/></div>
            <h3 className="text-xl font-black text-slate-800 dark:text-slate-100 mb-2">削除しますか？</h3>
            <p className="text-sm text-slate-400 dark:text-slate-500 font-medium mb-8 leading-relaxed">この操作は取り消せません。{isFolder && "グループ内のお薬もすべて削除されます。"}</p>
            <div className="flex gap-3">
              ...
              <button onClick={() => { onDelete?.(initialData!.id); setShowDeleteModal(false); }} className="flex-1 py-4 bg-red-500 text-white rounded-2xl font-bold shadow-lg shadow-red-200">削除する</button>
            </div>
          </div>
        </div>
      )}
