...
const SEVERITY_STYLE: Record<InteractionSeverity, { label: string; className: string }> = {
  high: { label: '注意度: 高', className: 'bg-red-50 text-red-600 border-red-200' },
  medium: { label: '注意度: 中', className: 'bg-amber-50 text-amber-600 border-amber-200' },
  low: { label: '注意度: 低', className: 'bg-blue-50 text-blue-600 border-blue-200' },
};
...
export const InteractionCheckModal: React.FC<InteractionCheckModalProps> = ({ medications, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[300] bg-black/50 backdrop-blur-sm flex items-end justify-center">
      <div className="w-full max-w-md bg-white rounded-t-[32px] p-6 pb-8 safe-bottom max-h-[85vh] overflow-y-auto no-scrollbar animate-in slide-in-from-bottom duration-300">
              ...
              result.pairs.map((pair, idx) => (
                <div key={idx} className="p-4 rounded-2xl border border-slate-100 bg-slate-50 space-y-2">
                  <div className="flex items-center justify-between">
                    <p className="font-black text-sm text-slate-800">{pair.medA} × {pair.medB}</p>
                  ...
                  <p className="text-xs text-slate-600 leading-relaxed">{pair.description}</p>
                </div>
              ))
            )}

            {result.summary && (
              <p className="text-xs text-slate-500 leading-relaxed bg-slate-50 p-4 rounded-2xl">{result.summary}</p>
            )}
